...
from tensorflow.keras.layers import Activation, Add, BatchNormalization, Conv2D, Dense, Dropout, Layer, Multiply
from layers import Mean, Max, Expand, Squeeze


def get_particle_net(num_constituents, num_points, config):
    """
    ...
    outputs = _particle_net_base(points, features, mask, coord_shift, config)

    model = Model(inputs=[points, features, mask, coord_shift], outputs=outputs)
...
def _particle_net_base(points, features, mask, coord_shift, config):
    """
    points : (N, P, C_coord)
